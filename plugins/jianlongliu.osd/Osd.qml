...
Item {
  ...
  PanelWindow {
    id: panel
    visible: root.opened
    // Card-sized surface, centered at the bottom edge, so Hyprland blurs only
    // the card. A fullscreen transparent surface blurs the whole screen, and
    // ignore_alpha can no longer mask it on current Hyprland, so the surface
    // must not span the screen.
    ...
    margins {
      ...
      right: (Quickshell.screens[0].width - (card.borderLeft + root.pad + root.contentWidth + root.pad + card.borderRight)) / 2
    }
    color: "transparent"
