...
Panel {
  ...
  // Status icons come from Material Symbols as ligature names (see iconFont).
  readonly property string icon: {
    if (!adapter) return ""
    if (!adapter.enabled) return "bluetooth_disabled"
    if (connectedDevices.length > 0) return "bluetooth_connected"
    return "bluetooth"
  }

  // Material Symbols Outlined ships in ~/.local/share/fonts. Switch this back
  // to root.bar.fontFamily and the Nerd glyphs above to revert.
