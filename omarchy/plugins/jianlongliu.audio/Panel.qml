...
Panel {
  ...
  // Same ladder as outputIcon() below, but returning a Material Symbols
  // ligature name for the bar button and panel hero. outputIcon() keeps
  // feeding Nerd glyphs to the OSD.
  function outputIconName(volume) {
    if (!sink || !sink.audio) return "volume_off"
    if (isHeadphones(sink)) return "headphones"
    if (outputMuted) return "volume_off"
    var v = volume === undefined ? outputVolume : volume
    if (v >= 0.67) return "volume_up"
    if (v >= 0.34) return "volume_down"
    if (v > 0) return "volume_mute"
    return "volume_off"
  }

  // Material Symbols Outlined ships in ~/.local/share/fonts. Switch this back
  // to root.bar.fontFamily to revert.
