...
| 升级后重贴（2026-09-24 已做） | 已重贴并对齐 v1.8.1。**结果与旧版像素完全一致**（`spin*` / `dot` / `bg` 逐张 `magick compare -metric AE` = **0**）——因为上游 v1.8.1 的改动（96px 圆弧、`entry.ih`）只作用于**回退分支**，本机走竖线分支。**且 `generate.sh` 是生成器、不随主题装进 initrd**（initrd 里只有 `bg*.png` / `dot.png` / `spin*.png` / `omarchy-boot.script` / `.plymouth` / `design`）→ 重贴不改变任何引导产物，纯粹是版本对齐。耗时：重建两个 UKI 约 40s |
